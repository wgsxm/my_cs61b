public class ArrayDeque <T>{
    ...
    private int front;
    private int back;
    private static final int originalCapacity = 8;
    /**
     * create a empty deque
     */
    public ArrayDeque()
    {
        items=(T[]) new Object[originalCapacity];
        front=originalCapacity-1;
        back=0;
        size=0;
    }
    ...
    {
        if(back>front)this.resize(this.size()*2);
        size++;
        items[front]=item;
        front--;
    }
    ...
    public T removeFirst()
    {
        ...
        T ret;
        if(front<items.length-1)ret=items[front+1];
        else {
            ret=items[0];
        }
        size--;
